...
export const BestSeller = ({bestSellers}) => {
  const [selectedBook, setSelectedBook] = useState(1)
  const result = bestSellers.filter(book => book.id === selectedBook)

  return (
    <section>
      <h2 className='font-bold text-lg leading-7 text-center text-orange-500 pt-5 pb-2'>Mais Vendidos</h2>
      <div className='bg-gray-300 pb-7'>
        <div className='flex pt-2 pb-12'>
          {bestSellers.map((item: ILastReleases) => 
            <img 
              src={item.imagemCapa} 
              alt="Capa do livro" 
              key={item.id} 
